...
lda_pred <- predict(lda_mod, clean_testing)
```

```{r random_forest}
rf_mod <- train(classe ~ ., method = "parRF", data = clean_training)
rf_pred <-predict(rf_mod, clean_testing)
```

```{r gradient_boosting}
xgbTree_mod <- train(classe ~ ., method = "xgbTree", data = clean_training)
xgbTree_pred <- predict(xgbTree_mod, clean_testing)
```


```{r support_vector_machines}
svm_mod <- train(classe ~ ., method = "svmLinear", data = clean_training)
svm_pred <- predict(svm_mod, clean_testing)
```


```{r stacked}
stacked <- data.frame(lda_pred, rf_pred, xgbTree_pred, svm_pred, classe = testing$classe)
stacked_mod <- train(classe ~ ., method = "rf", data = stacked)
stacked_pred <- predict(stacked_mod, clean_testing)
```
...
```{r}
confusionMatrix(lda_pred, clean_testing$classe)
```

###Random Forest Performance:
```{r}
confusionMatrix(rf_pred, clean_testing$classe)
```

### Gradient Boosting Performance:
```{r}
confusionMatrix(xgbTree_pred, clean_testing$classe)
```

###Support Vector Machines Performance:
```{r}
confusionMatrix(svm_pred, clean_testing$classe)
```

###Stacked Model Performance:
```{r}
confusionMatrix(stacked_pred, clean_testing$classe)
```
