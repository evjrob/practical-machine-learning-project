...
It is unlikely that all 38 columns from each accelerometer will be necessary to properly classify each excersice, and so we will remove or consolidate columns that are unlikely to be crutial to classification.

# Remove sparse columns

# Remove low variance columns or collapse them into fewer columns using PCA
...
pre_process_data <- function(data) {
  drop_columns <- c(-1, # Drop column "X" since it is just a row index.
                    -2, # Drop column "user_name" since the predictions should work independent of user
                    -3,
                    -4, 
                    -5,
                    -6,
